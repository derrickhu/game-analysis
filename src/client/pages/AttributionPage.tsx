...
export function AttributionPage() {
  ...
  const dailyColumns: ColumnsType<(typeof dailyCohorts)[number]> = [
    ...
    { title: '付费/已知', dataIndex: 'paid_or_known_users', key: 'paid', align: 'right' },
    ...
    <Space orientation="vertical" size="middle" style={{ width: '100%' }}>
        ...
        <Space wrap>
          <RangePicker
            ...
            presets={[
              ...
              { label: '近 30 天', value: defaultAttributionRange() },
            ]}
          />
          <Text type="secondary">
            按用户首次出现日期（注册 cohort）筛选，默认近 30 天且不含今天。顶部「近 30 分钟」等实时窗口不适合本页，请用此处日历范围。
          </Text>
        </Space>
      </Card>

      <Card
        title="广告归因分析（首期 dry-run）"
        extra={
          <Space>
      ...
      >
        <Alert
          ...
          description="上方 KPI 与下表均按「分析范围」内的注册 cohort 统计，不是全游戏历史存量。微信开发者工具可用 mock query 验证工程链路。"
          style={{ marginBottom: 16 }}
        />
        <Row gutter={[16, 16]}>
          <Col xs={12} md={4}>
            <Statistic title="期间新增（已归因）" value={summary?.attributed_users || 0} />
          </Col>
          <Col xs={12} md={4}><Statistic title="付费/已知来源" value={summary?.paid_or_known_users || 0} /></Col>
      ...
      <Card title="每日新增归因（按注册日）" loading={loading && !data}>
      ...
      <Card title="Campaign / Adgroup / Creative 质量排行（期间注册 cohort）" loading={loading && !data}>
        {rankings.length > 0 ? (
          <Table
            rowKey="key"
            size="small"
