 ...
 *
 * - 埋点表 analytics_events.platform：wechat / douyin / h5 / unknown
 * - 玩家档案 user_id 前缀：wx / dy / h5 / anon（另一套历史编码）
 * - 云存档集合：`{gameKey}_playerData`（微信）/ `{gameKey}_tt_playerData`（抖音）
 * - 归因表 platform：广告投放商（tencent_ads），与本模块无关
 *
 * 全局筛选只提供微信 / 抖音：混算会把两端量级糊在一起，留存/对比易误读。
 */

export type AnalyticsPlatform = 'wechat' | 'douyin' | 'h5';

/** 全局筛选值：必须落到具体平台（默认微信） */
export type PlatformFilter = 'wechat' | 'douyin';
...
const UI_VALID = new Set<string>(['wechat', 'douyin']);
 ...
 *
 * 经分 gameKey 永远用基础名（huahua / hotpot / petTower），不要带 `_tt`。
 */
